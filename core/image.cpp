...
void Image::_allocResources()
{
	...
	{
	case Img_Pixel_RGBF:
		_fiBitmap = FreeImage_AllocateT(FIT_RGBF, _Width, _Height);
		break;
	case Img_Pixel_RGBAF:
		_fiBitmap = FreeImage_AllocateT(FIT_RGBAF, _Width, _Height);
		break;
	default:
	...
	_bpp = FreeImage_GetBPP(_fiBitmap);
}
